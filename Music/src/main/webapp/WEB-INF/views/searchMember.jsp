...
<link rel="stylesheet" href="./resources/css/searchMember.css" />
</head>
<body>
	...
	<div class="header">
		Searching with login
	</div>
	<form action="searchMemberOk" method="post" onsubmit="return formCheck();">
	...
	<div align="right" style="width: 75%; margin: 5px;">
		<button type="button" class="btn btn-secondary" name="addManyBtn" onclick="addManyBtn()" style="width: 200px;">Add checked musics</button>
	</div>
	<div align="right" style="width: 75%; margin: 5px;">
		<button type="button" class="btn btn-secondary" id="musicCart" style="width: 200px;">My Music</button>
	</div>
			...
			<c:if test="${not empty list1 }">
				<c:forEach var="vo" items="${list1 }">
					<tr>
						...
						<td class="addCart" align="center">
							<%-- 이부분을 반드시 숨겨서 가져가야 한다. --%>
							<input type="hidden" class="csrf" name="${_csrf.parameterName}" value="${_csrf.token}" />
							<button type="button" class="btn btn-outline-secondary btn" name="addBtn" onclick="addBtn(${vo.music_idx})">+</button>
						</td>	
					</tr>
				</c:forEach>
			</c:if>
